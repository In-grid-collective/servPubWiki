...
1. The server package is installed on the SBC/Pi:
...
We want to change the default PW for the pi, if we have not already set one we want to keep during set up.
...
Notes *You might need to install an ssh client server package for each, still need to verify this on windows machines: